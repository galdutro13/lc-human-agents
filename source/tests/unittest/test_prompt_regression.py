...
from source.simulation_config import carregar_config_v43, montar_prompt


ROOT = Path(__file__).resolve().parents[3]
V43_PATH = ROOT / "config_v4_3.json"
LEGACY_V3_PATH = ROOT / "source/tests/fixtures/legacy/config_v3.json"
MAPPING_PATH = ROOT / "source/tests/fixtures/legacy/prompt_mission_map_v43.json"
SPECIAL_CASES = {
...
class TestPromptRegressionV43(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config_v43 = carregar_config_v43(V43_PATH)
    ...
    def test_personas_originais_preservam_identidade_e_como_agir(self):
        for persona_id in self.mapping:
            self.assertEqual(
                self.config_v43["personas"][persona_id]["identidade"],
                self.legacy_v3["personas"][persona_id]["identidade"],
            )
            self.assertEqual(
                self.config_v43["personas"][persona_id]["como_agir"],
    ...
    def test_17_prompts_mantem_string_match_exato(self):
        for persona_id, missao_id in self.mapping.items():
            if persona_id in SPECIAL_CASES:
                continue
            prompt_v43 = montar_prompt(
                self.config_v43["personas"][persona_id],
                self.config_v43["missoes"][missao_id],
                self.config_v43["template_prompt"],
            )
            ...
            self.assertEqual(prompt_v43, prompt_v3)
    ...
    def test_3_prompts_mudados_tem_mapeamento_explicito(self):
        for persona_id, missao_id in SPECIAL_CASES.items():
            ...
            self.assertNotEqual(prompt_v43, prompt_v3)
            self.assertIn(self.config_v43["missoes"][missao_id]["missao"], prompt_v43)
