...
As a proof of concept, we provide a canonical v4.4 configuration for synthetic dialogue generation in a banking credit-card context.
...
The `--prompts-file` flag accepts only the versioned v4.4 configuration:
- `config_v4_4.json`: canonical schema with personas, missions, synthetic calendar parameters, parametric temporal factors, and runtime quota allocation for the configured `n`
...
coverage run -a -m unittest source.tests.integratio_test.test_v44_loader_pipeline_integration
