...
As a proof of concept, we provide a canonical v4.3 configuration for synthetic dialogue generation in a banking credit-card context.
...
The `--prompts-file` flag accepts only the versioned v4.3 configuration:
- `config_v4_3.json`: canonical schema with personas, missions, synthetic calendar, parametric temporal factors, and runtime quota allocation for the configured `n`

```bash
python ./tools/enxame_usuario/start_usuarios.py --prompts-file config_v4_3.json
```

You can also inspect the generated sampling plan without running any bots:

```bash
python ./tools/enxame_usuario/export_simulation_preview.py --config-file config_v4_3.json
python ./tools/enxame_usuario/export_simulation_audit.py --config-file config_v4_3.json
```
...
coverage run -a -m unittest source.tests.integratio_test.test_v43_loader_pipeline_integration
